...
function extractTitle(text) {
  // 특보 키워드 추출
  const keywords = ['폭염경보', '폭염주의보', '한파경보', '한파주의보', '대설경보', '대설주의보',
    '호우경보', '호우주의보', '강풍경보', '강풍주의보', '풍랑경보', '풍랑주의보',
    '태풍경보', '태풍주의보', '건조경보', '건조주의보', '황사경보', '황사주의보',
    '미세먼지', '초미세먼지', '오존', '자외선'];
...
function extractRegion(text) {
  const regions = ['경기', '서울', '인천', '강원', '충북', '충남', '대전', '세종',
...
function sortByGyeonggi(alerts) {
  return alerts.sort((a, b) => {
    const aIsGyeonggi = a.region?.includes('경기') || a.message?.includes('경기') ? 1 : 0;
    const bIsGyeonggi = b.region?.includes('경기') || b.message?.includes('경기') ? 1 : 0;
