...
function OotdGenerator({ selectedRegion }) {
  ...
  const generateFashionPrompt = (climate, genderVal, ageVal, styleVal) => {
    ...
    if (temp >= 33) {
      temperatureOutfit =
        "very light summer clothes, tank top, shorts, sandals";
      season = "hot summer";
    } else if (temp >= 28) {
      temperatureOutfit =
        "light summer outfit, short sleeve t-shirt, thin pants or shorts";
      season = "summer";
    } else if (temp >= 23) {
      temperatureOutfit =
        "light spring outfit, thin long sleeve or short sleeve, light pants";
      season = "late spring";
    } else if (temp >= 17) {
      ...
      temperatureOutfit =
        "winter outfit, padded jacket, warm coat, beanie, gloves";
      season = "winter";
    }
    ...
    const ageText =
      AGE_OPTIONS.find((a) => a.value === ageVal)?.prompt || "young adult";
    const styleText =
      STYLE_OPTIONS.find((s) => s.value === styleVal)?.prompt || "casual";
  ...
  const generateDescription = (
    ...
    desc.push(
      emojis.tops + " 상의: " + recommendation.tops.slice(0, 3).join(", "),
    );
    desc.push(
      emojis.bottoms +
        " 하의: " +
        recommendation.bottoms.slice(0, 3).join(", "),
    );
    desc.push(
      emojis.shoes + " 신발: " + recommendation.shoes.slice(0, 3).join(", "),
    );
    desc.push(
      emojis.accessories +
        " 아이템: " +
        recommendation.accessories.slice(0, 3).join(", "),
    );
    ...
    if (humidity >= 80) {
      desc.push(
        "💧 습도 매우 높음 - 땀 흡수 빠른 기능성 소재, 린넨/면 필수! 여분의 옷 챙기세요.",
      );
    } else if (humidity >= 70) {
    ...
    if (pm10 >= 150 || pm25 >= 75) {
      desc.push(
        "😷 미세먼지 매우 나쁨 - KF94 마스크 필수! 가급적 외출 자제하세요.",
      );
    ...
    if (uvIndex >= 11) {
      ...
      desc.push(
        "🕶️ 자외선 매우 높음 - 선크림, 선글라스, 모자 필수! 한낮 외출 자제",
      );
    } else if (uvIndex >= 6) {
    ...
    if (recommendation.tips && recommendation.tips.length > 0) {
      const randomTip =
        recommendation.tips[
          Math.floor(Math.random() * recommendation.tips.length)
        ];
      ...
      {isOpen && (
        <div className="ootd-panel-content">
          ...
          <div className="weather-summary">
            <span>
              🌡️{" "}
              {selectedRegion.climate_data?.apparent_temperature ||
                selectedRegion.climate_data?.temperature}
              °C
            </span>
          ...
          {/* 에러 표시 */}
          {error && <div className="ootd-error">{error}</div>}
