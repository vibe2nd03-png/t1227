...
const OUTDOOR_SPOTS = {
  '가평군': [
    ...
    { name: '아침고요수목원', type: 'park', description: '사계절 아름다운 정원' },
    { name: '남이섬', type: 'park', description: '가족 나들이 명소' },
  ],
  '양평군': [
    { name: '두물머리', type: 'park', description: '한강과 북한강이 만나는 곳' },
  ...
  '파주시': [
    { name: '헤이리 예술마을', type: 'culture', description: '예술과 카페의 거리' },
...
const INDOOR_ALTERNATIVES = {
  ...
  '실내체험': [
    { name: '경기도어린이박물관', region: '용인시', description: '체험형 어린이 박물관' },
    { name: '국립과천과학관', region: '과천시', description: '과학 체험 전시' },
    { name: '롯데월드 아쿠아리움', region: '수원시', description: '해양생물 관람' },
  ],
  '쇼핑몰': [
...
const getAirQualityGrade = (pm10, pm25) => {
  const pm10Grade = pm10 <= 30 ? 'good' : pm10 <= 50 ? 'normal' : pm10 <= 100 ? 'bad' : 'veryBad';
  const pm25Grade = pm25 <= 15 ? 'good' : pm25 <= 25 ? 'normal' : pm25 <= 50 ? 'bad' : 'veryBad';
...
function AirQualityNav({ climateData, onRegionSelect, isModal = false }) {
  ...
  const cleanZoneRanking = useMemo(() => {
    ...
    return climateData
      .map((region) => ({
        ...region,
        airScore: (region.climate_data?.pm10 || 50) + (region.climate_data?.pm25 || 25) * 2,
  ...
  const recommendedCourses = useMemo(() => {
    ...
    return DRIVE_COURSES.map((course) => {
      const cleanCount = course.regions.filter((r) => cleanRegions.includes(r)).length;
      const avgAirScore = course.regions.reduce((sum, r) => {
          ...
          {activeTab === 'clean' && (
            <div className="tab-content clean-zones">
              ...
              <div className="zone-list avoid">
                ...
                {cleanZoneRanking.filter((z) => z.grade === 'bad' || z.grade === 'veryBad').length === 0 && (
                ...
                {recommendedCourses.map((course, idx) => (
                  ...
                  >
                    <div className="course-header">
                      {course.recommended && <span className="rec-badge">추천</span>}
                      <h5>{course.name}</h5>
                      ...
                      <div className="course-detail">
                        ...
                        <div className="course-regions">
                          {course.regions.map((region) => {
                            ...
                            return (
                              <span key={region} className={`region-chip ${grade}`}>
